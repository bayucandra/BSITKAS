...
void CutiDaftarPegawaiDialog::InitDataView(){
...
void CutiDaftarPegawaiDialog::OnCari( wxCommandEvent& event ){
    RefreshDataView();
}
void CutiDaftarPegawaiDialog::RefreshDataView(){
    if(conn.connected()){
...
void CutiDaftarPegawaiDialog::OnPilih( wxDataViewEvent& event ){
    Pilih();
}
void CutiDaftarPegawaiDialog::Pilih(){
    if(pegawai_dataViewListCtrl->HasSelection()){
        int selected_row=pegawai_dataViewListCtrl->GetSelectedRow();
        ((CutiDialog*)GetParent())->PilihPegawai(
